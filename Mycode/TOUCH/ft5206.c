		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			for(i=0;i<5;i++)
			{
				if(status&(1<<i))		//触摸有效?
				{
					...
					if((buf[0]&0XF0)!=0X80)pre_x=pre_y=0;//必须是contact事件，才认为有效
					printf("x:%d,y:%d\r\n",pre_x, pre_y);
				}			
			} 
			res=1;
			if(pre_x==0 && pre_y==0)mode=0;	//读到的数据都是0,则忽略此次数据
...
uint8_t get_one_point(void)
{
	uint8_t reg = 0;
	uint8_t buf[4];
	FT5206_RD_Reg(FT_REG_NUM_FINGER,&reg,1);//读取触摸点的状态  
